...
def export_pdf(data: dict, output_dir: str) -> str:
    ...
    filename = os.path.join(output_dir, f"macatron_SL{sl}_{safe}.pdf")
    ...
    story.append(Paragraph("MAC-A-TRON", title_style))
    story.append(Paragraph("Local Security Assessment Summary", sub_style))
    ...
    story.append(Paragraph(
        "Generated by MAC-A-TRON — local security posture workbench for macOS | "
        "github.com/ZombieDuckling/MAC-A-TRON | Authorized assessment only.",
        footer_style))
...
def export_html(data: dict, output_dir: str) -> str:
    ...
    os.makedirs(output_dir, exist_ok=True)
    safe = tgt.replace("https://", "").replace("http://", "").replace("/", "_").replace(".", "_")
    filename = os.path.join(output_dir, f"macatron_SL{sl}_{safe}.html")
    vuln_rows = ""
    for v in data["vulns"]:
...
<head>
<meta charset=\"UTF-8\">
<title>MAC-A-TRON Report — {tgt}</title>
<style>
*{{box-sizing:border-box;margin:0;padding:0}}
body{{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;background:#f6f4ef;color:#1f2933;padding:30px}}
.container{{max-width:960px;margin:auto;background:#fff;border:1px solid #d9d6cc;padding:28px}}
.header{{border-left:5px solid #0d4f68;padding-left:16px;margin-bottom:30px}}
.header h1{{font-size:2.0em;color:#0d4f68;letter-spacing:.04em}}
.header p{{color:#607080;font-size:.95em}}
.meta-grid{{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:30px}}
.meta-card{{background:#f3f6f8;border:1px solid #d8dee6;padding:14px}}
.meta-card .label{{font-size:.75em;color:#607080;text-transform:uppercase;margin-bottom:4px}}
.meta-card .value{{font-size:1.05em;font-weight:bold}}
.risk{{color:{rc}}}
section{{margin-bottom:30px}}
section h2{{font-size:1.15em;color:#0d4f68;border-bottom:1px solid #d8dee6;
            padding-bottom:8px;margin-bottom:14px}}
table{{width:100%;border-collapse:collapse;font-size:.88em}}
th{{background:#e8edf1;color:#334155;text-align:left;padding:10px;
    font-size:.8em;text-transform:uppercase;border-bottom:1px solid #cbd5df}}
td{{padding:10px;border-bottom:1px solid #e5e7eb;vertical-align:top}}
tr:hover td{{background:#f8fafc}}
code{{background:#eef2f6;padding:2px 6px;border-radius:3px;
      font-family:monospace;font-size:.85em;color:#25313f}}
.ai-box{{background:#f8fafc;border:1px solid #d8dee6;padding:16px;font-size:.9em;line-height:1.7;color:#25313f}}
.ai-box p{{margin-bottom:8px}}
.footer{{text-align:center;color:#667085;font-size:.78em;
         margin-top:40px;border-top:1px solid #d8dee6;padding-top:16px}}
a{{color:#0d4f68}}
</style>
</head>
<body>
...
<div class=\"header\">
  <h1>MAC-A-TRON</h1>
  <p>Local Security Assessment Summary</p>
</div>
...
<div class=\"footer\">
  Generated by MAC-A-TRON &mdash;
  <a href="https://github.com/ZombieDuckling/MAC-A-TRON">github.com/ZombieDuckling/MAC-A-TRON</a>
  &mdash; Authorized assessment only.
</div>
...
def export_menu(data: dict):
    ...
    output_dir = os.path.expanduser("~/MAC-A-TRON/reports")
    os.makedirs(output_dir, exist_ok=True)
...
if __name__ == "__main__":
    print("\n\033[36m    MAC-A-TRON — Standalone Report Exporter\033[0m")
